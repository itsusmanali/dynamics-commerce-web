...
export function createCommerceApi(config: CommerceConfig, auth: AuthHooks = {}) {
  const root = config.apiBaseUrl.replace(/\/$/, "");
  const api = axios.create({ baseURL: root.endsWith("Commerce") ? root : `${root}/Commerce`, timeout: 10_000, params: { "api-version": config.apiVersion }, headers: { Accept: "application/json", "Content-Type": "application/json", prefer: "return=representation", "odata-version": "4.0", "odata-maxversion": "4.0", OUN: config.oun } });
