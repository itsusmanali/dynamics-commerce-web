...
export default async function Category({ params }: Props) { const { slug } = await params; const path = `/blog/category/${slug}`; const [posts, item] = await Promise.all([getPostsByTaxonomy("category", slug), getTaxonomyBySlug("category", slug)]); return <><SeoSchema seo={item?.seo} path={path} /><PostList posts={posts} heading={item?.name || `Category: ${slug}`} /></>; }
