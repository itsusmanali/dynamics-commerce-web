...
export default async function Tag({ params }: Props) { const { slug } = await params; const path = `/blog/tag/${slug}`; const [posts, item] = await Promise.all([getPostsByTaxonomy("tag", slug), getTaxonomyBySlug("tag", slug)]); return <><SeoSchema seo={item?.seo} path={path} /><Breadcrumbs seo={item?.seo} path={path} /><PostList posts={posts} heading={item?.name || `Tag: ${slug}`} /></>; }
